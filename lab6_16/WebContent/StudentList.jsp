...
<a id="createButton" class="btn btn-primary pull-right" 
   href="StudentCreate.jsp?pg=<%=currentPage%>&srchText=<%=srchTextEncoded%>">
...
<form class="form-inline">
  <div class="form-group">
    <label>이름</label>
    <input type="text" class="form-control" name="srchText" value="<%= srchText %>" //자동인코딩
           placeholder="검색조건" />
  </div>
...
<table class="table table-bordered table-condensed">
  ...
  <tbody>
    <% for (Student student : list) { %>
      <tr data-url="StudentEdit.jsp?id=<%=student.getId()%>&pg=<%=currentPage%>&srchText=<%=srchTextEncoded%>">
        <td id="mouse"><%= student.getId() %></td>
