...
<%@ page import=" java.net.*, java.util.*, lab6_16.*" %>
<%
...
if (request.getMethod().equals("GET")) {
    student = StudentDAO3.findOne(id);
}
else {
    student = new Student();
    student.setId(id);
    ...
    if (s1 == null || s1.length() == 0) 
        ...
        response.sendRedirect("StudentList3.jsp?pg=" + pg + "&ss=" + ss + "&st=" + stEncoded + "&od=" + od);
        return;
    }
}
%>
...
<form method="post">
  ...
  <a href="StudentDelete3.jsp?id=<%= id %>&pg=<%= pg %>&ss=<%=ss%>&st=<%=stEncoded%>&od=<%=od%>" 
     class="btn btn-danger" onclick="return confirm('삭제하시겠습니까?')">
    <i class="glyphicon glyphicon-trash"></i> 삭제
  </a>
  <a href="StudentList3.jsp?pg=<%= pg %>&ss=<%= ss %>&st=<%=stEncoded%>&od=<%=od%>" 
