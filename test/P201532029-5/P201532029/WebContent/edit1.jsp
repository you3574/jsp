...
<%@ page import="java.net.*, java.util.*, p201532029.*" %>
<%
...
<div class="form-group">
    <label>제목</label>
    <input type="number" class="form-control" name="year" value="<%= article.getTitle() %>" />
  </div>

  <div class="form-group">
    <label>본문</label>
    <textarea class="form-control" name="body" rows="20" value="<%= article.getBody() %>"/></textarea>
  </div>
  <div class="form-group">
  ...
  <div class="checkbox">
      <label><input type="checkbox" name="chk" value="1" /> 공지</label>
    </div> 
  <button type="submit" class="btn btn-primary">
    <i class="glyphicon glyphicon-ok"></i> 저장
  </button>
  <a href="list1.jsp?id=<%= id %>&pg=<%= pg %>&ss=<%=ss%>&st=<%=stEncoded%>&od=<%=od%>" 
